...
- For the sake of brevity, the results of only two nine-factor `Promax` solutions are displayed below.  Each has passed certain key model fit and consistency/reliability tests:
    ...
    - Each solution was evaluated for *overall* consistency/reliability using Cronbach's Alpha, Greater Lower Bound, and Omega; and each scored greater than 0.70 on each of these tests
...
```{r PLAYGROUND-CODESWITCH2,  fig.align='center', fig.dim= c(16,12) }
...
cs$metric = c('Sex=F', "Height < 5'7", 'Training < Advanced', 'Race=Non-White')
...
ggplot( ., aes(x = OR.EXP, y = factor(metric, level = metric)) ) +
  ...
  geom_text(data = ~., aes(label = paste(round(OR.EXP,2), signif, sep= " "), 
                          y = metric, 
                          x= OR.EXP+0.15), size=4, color="dodgerblue", fontface="bold.italic", nudge_y= 0.15, nudge_x= 0.25)+
...
- If we were to try to use odds ratios to predict the profile of these so-called "Code Switchers," we might find that the . . ..
    ...
    - The odds of them being either a First- or Second-Year trainnee is 2.71
