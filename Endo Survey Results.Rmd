...
#### <span style="color:red"> [Click here for the latest version of the Summary Tables (downloadable Word document)](https://github.com/boskev/endogendersurvey/blob/main/Ergonomics%20tables%20questions%209-21.docx)</span>

<br>
<br>

#### <span style="color:red">[Click here to jump to the latest Oblique Factor Analysis Solutions](#Oblique)</span>
...
#### Do we have a large enough sample to adequate "power" our analysis?
...
- A 2 x 2 Chi-Square (one degree of freedom): e.g., `BIRTHSEX` x a two-level categorical variable (Y/N)
- A 2 x 3 Chi-Square (two degrees of freedom): e.g., `BIRTHSEX` x a three-level categorical variable (Y/N/Don't Know)
- A 2 x 4 Chi-Square (three degress of freedom): e.g., `BIRTHSEX` x a four-level categorical variable (Training Level)
- A 2 x 6 Chi-Square (five degrees of freedom): e.g., `BIRTHSEX` x a six-level categorical variable (Race)

<br>
<br>

```{r COHEN-1, fig.align='center', warning= F, fig.dim= c(16,16)}

suppressMessages(detach("package:flextable", unload=F))
require(effsize)
require(pwr)
...
```{r COHEN-2, fig.align='center', fig.dim= c(16,16), warning=F}
...
(Note that the results for multivariate regression could not be graphed, but according to the `cohen.ES` function from the `effsize` package, a regression with eight covariates (not likely, but assumed here for conservatism), requires about 126 subjects to achieve a "medium effect."
...
#### Follow-Up Question:  Is there a difference in Injury reports based on *Training Level?*

<br>

```{r CROSSTAB-BIRTHSEX-INJURY_SPECIFIC-FOLLOWUP }
...
- **Interpretation:**  `TRAINING_LEVEL` is *not* a significant effect in predicting the likelihood of Injury `EVER_INJURED`.  Despite the facts that (1) the prevalence of self-reported injuries *increases* with each step of `Training Level` (First Years are the least likely to be injured (odds=1.09 or 9%), compared to Second Years (odds=1.23 or 23%) and Advanced respondents (odds=1.22 or 23%); and (2) Second Years & Advanced respondents have a higher likelihood of injury when compared against all other groups, none of these results is statistically significant.  
...
```{r CROSSTAB-BIRTHSEX-KNOWLEDGE-DISABILITY-INSURANCE, warning= F }
...
#### Ergo Quiz Score Results by BIRTHSEX and by TRAINING LEVEL:

<br>

```{r CROSSTAB-ERGO-QUIZ-RESULTS, warning= F }
...
#bartlett.test( COUNT_CORRECT ~  TRAINING_LEVEL , data = CORRECT3)
#Anova( lm( COUNT_CORRECT ~ TRAINING_LEVEL, data=CORRECT), type=3)

#Welch's Anova - to match ggbetween stats (unequal variances assumed in plot)
oneway.test(COUNT_CORRECT ~ TRAINING_LEVEL, data = CORRECT, var.equal = FALSE)
...
#Welch's Anova - to match ggbetweenstats (unequal variances assumed in plot)
oneway.test(COUNT_CORRECT ~ TRAINING_LEVEL, data = CORRECT3, var.equal = FALSE)
...
- **Conclusion:  ** When adjusted for multiple comparisons, the means among the various levels of the variable `TRAINING_LEVEL` are not statistically significant.  This analysis holds true whether `TRAINING_LEVEL` is stratified by three levels (p=0.013) or by four levels (p=0.027).  The mean is lowest among First Year trainees, but after the second year, the mean seems to peak and level off.  It may be worth noting that if the mean for First Year trainees is contrasted against the mean *for all other trainees combined,* it is significant if `TRAINING_LEVEL` is a three-level variable (p=0.0416) but not significant if `TRAINING_LEVEL` is a four-level variable (p=0.0714).

<br>
<br>


#### Did subjects who self-reported having ERGO OPTIM knowlege do better on Quiz?

<br>

```{r CROSSTAB-ERGO-QUIZ-RESULTS2, warning= F }
...
```{r TASKS-CORRELATION-MATRIX, fig.align='center', fig.dim= c(16,12) }
...
###### There does not appear to be any association in this sample with HEIGHT or GLOVE and having sustained an injury or experienced any of the five specific transient pain types listed in the survey.  Does this change if we adjust the correlations based on sex?  

<br>

```{r TASKS-CORRELATION-PARITALS, warning=F}
require(ppcor)
...
- **Conclusion:**  Ergonomic Optimization is a significant effect before and after adjust for Training Level, which itself is *not* a significant effect. (The Odds Ratio in the adjusted model rose from 3.15 to 3.23, p= 0.0105.  In other words, subjects who reported that their Endoscopy Suite environment was not ergonomically optimized were 3.23 times more likely to also report having experienced Transient Pain in one of the several body parts highlghted in the survey.)
...
- **Conclusion:**  Sensitive Teacher is *not* a significant effect before (p=0.198) or after adjust for Training Level (p=0.217), which itself is *not* a significant effect. (In the adjusted model, the odds of experiencing Transient Pain if one did *not* have a Sensitive Teacher was 1.78, but with an insignificant p-value, cl=[0.713, 4.43])
...
```{r TASKS-CORRELATIONS-FNAME_NOPERM-EXPLORATORY-REGRESSION4 , fig.dim= c(12,6) } 
...
- Encouraged to Use Dial Ext (Yes=1, default=0)
- Pediatric Colonoscopes Available (Yes=1, default=0)
- LW 1P Aprons Available (Yes=1, default=0)
- LW 2P Aprons Available (Yes=1, default=0)
- Well-fitted Aprons Available (Yes=1, default=0)
- Monitors Easily Adjustable (Yes=1, No=0)
- Teachers Sensitivity During Training (Yes=1, No=0)
...
TRAINING %>% 
  group_by(BIRTHSEX) %>% 
  summarize( EQUIPM = mean(EQUIP_SCORE, na.rm=T),
             EQUIPSD = sd(EQUIP_SCORE, na.rm=T ))
...
```{r PLAYGROUND-RESPECT, warning=F}
...
#### Question 5:  What happens if we use <span style="color:red">Factor Analysis</span> as a means of data reduction and to confirm that the variables that we intuitively assume belong together actually load highly on the same factor?  
...
```{r PLAYGROUND-FACTANAL1, warning=F}
...
```{r PLAYGROUND-FACTANAL2, warning=F}
...
```{r PLAYGROUND-FACTANAL3, warning=F}
...
```{r PLAYGROUND-FACTANAL4, warning=F}
...
```{r PLAYGROUND-FACTANAL6, warning=F,  fig.align='center', fig.dim= c(16,12)}
...
```{r PLAYGROUND-FACTANAL7,   fig.align='center', fig.dim= c(16,12)}
...
#### <a name="Oblique"></a>Question 5A:  <span style="color:red">Factor Analysis Revisited: OBLIQUE (PROMAX) ROTATIONS</span>
...
- For the sake of brevity, the results of only two EFA `Promax` rotations are displayed below, one an eight-factor and the other a nine-factor solution.  Each has passed certain key model fit and consistency/reliability tests:

    - Each solution has an overall KMO greater than 0.67
    - Each solution explains at least 40% of the overall variance
    - Each solution has a Tucker-Lewis Index (TMI) greater than 0.90 and a Root Mean Standard Error Approximation (RMSEA) less than 0.3
    - Each solution was evaluated for *overall* consistency/reliability using Cronbach's Alpha, Greatest Lower Bound, and Omega; and each scored greater than 0.70 on each of these tests
    - The individual factors themselves for each solution were evaluated for consistency/reliability, and none scored lower than 0.60 on any of the three aforementioned tests (both solutions produced *only one factor* that was between 0.60 and 0.69 -- the remaining factors produced scores between 0.70 and 0.88, depending on the test)


<br>
<br>

#### Promax Solution 1:  8-Factors (41 of 54 variables included in final factor load)

<br>
<br>


![MLE Factor Analysis: Promax Rotation with 8 factors produced from 41 questions](images/Promax8-41Q.png)

<br>
<br>

![Correlation Matrix for Promax-8-DialExt](images/PA Factor Analysis - 8-Factor PROMAX Corrmatrix (Median Substitution DIALEXT).png)


<br>

- What makes this solution interesting is that it highlights just how much of what transpires in the Endoscopy Suite hinges on the factor tentatively named **Respect-Confidence** (Respect Endoscopy Suite Staff/Trainee Confidence).
- <b>Key Message:</b>  When there is Respect in the workplace, there is also training with sensitivity, nurses who respond on the first ask, heightened trainee confidence, and tendency to use lightweight, more comfortable aprons and dial extenders -- all correlating with a *decrease* in reports of transient pain and injury
- By expanding to eight factors, not only did we achieve optimal TMI and RMSEA values (i.e., better model fit compared to the Varimax rotation examples discussed previously), we saw a schism in one factor from one of our Varimax solutions:  "Apron Fit & Availability" produced two distinct, reliable factors related to "Standard Aprons" and "Lightweight Aprons."

<br>
<br>

##### Regressions using BIRTHSEX and select factors to predict likelihood for Pain/Injury

<br>

![Regressions:  BIRTHSEX predicting PAIN/INJURY](images/Promax 8 - Dial Ext Regressions.png){width=1600px}

<br>

- The regression summary above charts what happens to the coefficient of the variable BIRTHSEX when it is used to predict the factor PAIN.  Unadjusted, the coefficient starts at 0.46. The next five models show what happens if only *one* intervention is made:  For example, adding a *standard* apron increases the coefficient marginally to 0.47 (i.e., increases the likelihood of PAIN). But the next single-step interventions (red underscores) - using a lightweight apron, treating trainees with respect, having nurses be more cooperative, and training with sensitivity and appropriate feedback - each reduces the coefficient to as low as 0.36.   In the next models (blue underscores), we see what happens when we (1) include *all* the aforementioned interventions (coefficient drops to 0.34); (2) include the Tactile Training factor, which is *positively* correlated with the Pain factor (coefficient drops further to 0.29); and (3) adjust further for HEIGHT (which is so strongly correlated with BIRTHSEX that it must be included to confirm a true sex effect), and Training Level.  The final model, which includes all these covariates, has a coefficient of 0.30 and our variable of interest, BIRTHSEX, has maintained its significance throughout (whereas HEIGHT and TRAINING_LEVEL did not, according to the Type 3 Anova table, not shown).
- <b>Final Takeaway:</b>  Training with Sensitivity + Respect from Sr. ES Staff + Cooperative Nurses + Lightweight Aprons all contribute to significantly
reduce the likelihood of Pain relative to Sex, even after adjusting for Height and Training Level.


<br>
<br>

#### Promax Solution 2:  9-Factors (43 of 54 variables included in final factor load)

<br>
<br>


![MLE Factor Analysis: Promax Rotation with 9 factors produced from 43 questions](images/Promax9-43Q.png)

<br>
<br>

![Correlation Matrix For Promax-9-CodeSwitch](images/PA Factor Analysis - 9-Factor PROMAX Corrmatrix (Median Substitution CODESWITCH).png)


<br>

- This solution is very similar to the one above, except that it includes two questions that were originally ignored due to their perceived limited scope (i.e., questions regarding whether trainees were more or less likely to use Dial Extenders in front of a male or female instructor, as 70% of trainees reported not even using Dial Extenders).  By adding these two new questions for this solution and *reducing* the number of factors requested to nine factors, we were able to achieve even *better* model fit.  It should be noted that the scoring of these questions included *all* subjects, not just those reporting having used Dial Extenders -- so *every* respondent contributed in some way to the factor loadings and scores involving these new two questions.
- As a result of these changes, we may have stumbled upon an important factor/latent variable that can be best described as `"Code Switching."`  While the term is typically used in the context of cultural linguistics, "Code Switching" here is meant to imply that some trainees may act one way in front of one audience (i.e., "their people") and another way in front of perceived figures of authority (or simply in front of instructors who are different from them).
- Another way of looking at this:  Code Switchers are "people pleasers" who are not inclined to "rock the boat."  And according to the correlation matrix above, they do not appear to command respect from ES Staff, do not perceive nurses as cooperative, are not vocal about ill-fitting lightweight or standard issue aprons, do not feel that they are trained with sensitivity/adequate feedback (possibly because they do not insist on it), and as a result, they have high correlation with reported incidences of transient pain and injury.
...
```{r PLAYGROUND-CODESWITCH,   fig.align='center'}
...
- The factor `F4.CODE.SWITCH` is highly correlated with `BIRTHSEX` (females), and `HEIGHT` (shorter trainees), and less correlated with `TRAINING_LEVEL` (where, it seems to be more positively associated with First-Year & Second-Year trainees -- "Advanced" is the reference category, and as such, its bar appears first in the histogram).  It is *not* significantly correlated all with `RACE`, however (where RACE is a binary variable, Whites vs. Non-Whites, although it appears to trend higher among Non-Whites).

<br>
<br>

```{r PLAYGROUND-CODESWITCH2,  fig.align='center' }
...
rbind( 
      ...
      summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ BIRTHSEX , data=TEST, family=binomial(link='logit')), spec='BIRTHSEX', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,

            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ BIRTHSEX , data=TEST, family=binomial(link='logit')), spec='BIRTHSEX', reverse=T),
            #           list('Unadj OR: Females vs Males CODE.SWITCH=Y' = c( -1, 1)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
      ...
      summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ HEIGHT2 , data=TEST, family=binomial(link='logit')), spec='HEIGHT2', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ HEIGHT2 , data=TEST, family=binomial(link='logit')), spec='HEIGHT2', reverse=T),
            #         list("Unadj OR: 5'-5'6 vs 5'7-6'4 CODE.SWITCH=Y" = c( 0, 1/2, 1/2, -1/3 , -1/3, -1/3, 0)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
      ...
      summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ TRAINING_LEVEL , data=TEST, family=binomial(link='logit')), spec='TRAINING_LEVEL', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ TRAINING_LEVEL , data=TEST, family=binomial(link='logit')), spec='TRAINING_LEVEL', reverse=T),
            #         list('Unadj OR: First & Second Yr vs Advanced CODE.SWITCH=Y' = c( -1, 1/2, 1/2)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
      ...
      summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ RACE2 , data=TEST, family=binomial(link='logit')), spec='RACE2', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ RACE2 , data=TEST, family=binomial(link='logit')), spec='RACE2', reverse=T),
            #         list('Unadj OR: Non-Whites vs Whites CODE.SWITCH=Y' = c( -1, 1)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
    ...
    summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ F1.TRNG.FEEDBACK.SENSITIVITY , data=TEST, family=binomial(link='logit')), spec='F1.TRNG.FEEDBACK.SENSITIVITY', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ F1.TRNG.FEEDBACK.SENSITIVITY , data=TEST, family=binomial(link='logit')), spec='F1.TRNG.FEEDBACK.SENSITIVITY', reverse=T),
            #          list('Unadj OR: TRNG.FBACK.SENS= N &  CODE.SWITCH=Y' = c( 1, -1)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
    ...
    summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ F2.RESPECT.CONFIDENCE , data=TEST, family=binomial(link='logit')), spec='F2.RESPECT.CONFIDENCE', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ F2.RESPECT.CONFIDENCE , data=TEST, family=binomial(link='logit')), spec='F2.RESPECT.CONFIDENCE', reverse=T),
            #          list('Unadj OR: RESPECT.CONFIDENCE= N &  CODE.SWITCH=Y' = c( 1, -1)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
    ...
    summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ F3.NURSE.COOP , data=TEST, family=binomial(link='logit')), spec='F3.NURSE.COOP', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) ,
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ F3.NURSE.COOP , data=TEST, family=binomial(link='logit')), spec='F3.NURSE.COOP', reverse=T),
            #          list('Unadj OR: NURSE.COOP= N &  CODE.SWITCH=Y' = c( 1, -1)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE),
    ...
    summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ F9.PAIN , data=TEST, family=binomial(link='logit')), spec='F9.PAIN', reverse=T),
        ...
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)),
            ...
            # regrid(contrast(emmeans(glm( F4.CODE.SWITCH ~ F9.PAIN , data=TEST, family=binomial(link='logit')), spec='F9.PAIN', reverse=T),
            #           list('Unadj OR: TRANSIENT.PAIN= Y &  CODE.SWITCH=Y' = c( -1, 1)))) %>% tbl_df() %>% 
            #           mutate( OR.EXP = odds.ratio, LCL.EXP = OR.EXP-1.96*SE, UCL.EXP = OR.EXP+1.96*SE)     )


    summary(contrast(emmeans(glm( F4.CODE.SWITCH ~ F8.DIALEXT , data=TEST, family=binomial(link='logit')), spec='F8.DIALEXT', reverse=T),
                     list('Unadj OR: DIALEXT= Y &  CODE.SWITCH=Y' = c( -1, 1))), infer=T) %>%
        tbl_df() %>% mutate (OR.EXP = exp(estimate), LCL.EXP = exp(estimate - 1.96*SE), UCL.EXP = exp(estimate + 1.96*SE)) )


cs$metric = c('Sex= F', "Height < 5'7", 'Training < Advanced', 'Race= Non-White', "Trained w/Sensitivity = N",
              "Respected & Confident = N", "Nurses Cooperative = N", "Transient Pain = Y", "Uses Dial Extender = Y")
...
cs %>% arrange( (OR.EXP)) %>% 
  mutate ( OR.EXP = round(OR.EXP,2),
           yAxis = row_number(), 
           signif = case_when( LCL.EXP < 1 ~ 'NS',
                               p.value < 0.001 ~ "***",
                               p.value < 0.01 ~ "**",
                               p.value < 0.05 ~ "*",
                               p.value < 0.10 ~ "~",
...
- Since factors produced by EFA always have a mean of zero, it is simple to convert them to binary variables (where negative scores=0=No and positive scores=1=Yes), making running logistic regressions on these factors easy
- If we were to use the resulting odds ratios to inform the profile of these so-called "Code Switchers," we might find that they are . . . .

    - 5.21 times more likely to be **under 5' 7"**
    - 4.76 times more likely to be **Female**
    - 2.71 times more likely to be either a **First- or Second-Year trainee**
    - 6.41 times more likely to be **unconfident** or feel **disrespected by ES Staff**
    - 3.17 times more likely to feel that they were ***not*** **trained with sensitivity or with adequate feedback**
    - 1.93 times more likely to **view nurses as uncooperative**
    - 4.00 times more likely to **report incidences of transient pain** or outright injury
    - 1.83 times more likley to **use Dial Extenders**
    - 1.28 times more likely to be **Non-White** (insignificant results despite significant p-value - confint includes 1)
    
<br>    
- (In the above logistic regression calculations the `F4.CODE.SWITCH` factor is the *response* variable (Factor 4 = 1, or "Yes"), and odds ratios are computed relative to a corresponding reference category for each test)

<br>
<br>

![Regressions:  BIRTHSEX predicting PAIN/INJURY](images/Promax 9 - Code Switch Regressions.png){width=1600px}

<br>

- The regression summary is similar to the one used in the previous 9-factor PROMAX solution, where we try to confirm a SEX-PAIN association.  But in addition to controlling for HEIGHT, we now must contend with this new Code Switch factor, which is correlated with both of these covariates as well as with the outcome (PAIN).
- The first three models establish a baseline coefficient for each of these three variables: `BIRTHEX` = 0.45, `HEIGHTBAND` = 0.42 and `F4.CODE.SWITCH` = 0.32.
- The next two models adjust `BIRTHSEX` and `HEIGHTBAND` for `F4.CODE.SWITCH,` and in each case, while all variables remain statistically significant, the p-values for BIRTHSEX and HEIGHTBAND rose, while that of the Code Switch factor remained very low (p < 0.001).  In the next model, where all three variables have PAIN regressed on to them, *only* `F4.CODE.SWITCH` remains statistically significant (p < 0.001), suggesting that in this three-way battle, it is the Code Switch factor that is strongest predictor of transient pain/injury.
- Similar to what was done previously, the next models (blue underscore) add single interventions (ligthweight Aprons, Respect, Cooperative Nurses, etc.), each of which drops the coefficient for `F4.CODE.SWITCH` from its 0.32 high to as low as 0.20 (and this is with BIRTHSEX and HEIGHTBAND also included in the model as covariates).
- In the two "fully loaded" models -- one which adds Tactile Training, and which then adds Training Level -- the effect of `F4.CODESWITCH` remains statistically significant, and its coefficient drops as low as 0.13 (p= 0.027), even while controlling for *both* `BIRTHSEX` and `HEIGHTBAND`.

<br>

- <b>NOTE:  </b><br>The variable `HEIGHTBAND` (a binary variable, where 0=Short and 1=Tall) was used in the above regressions for the sake of simplicity.  (Having only two levels, `HEIGHTBAND` makes tracking the effects of height easier in these side-by-side model views without having to request a Type 3 Anova table.)  But if the *original* multilevel categorical variable `HEIGHT2` was used in these models instead -- similar to what was done with the previous Promax-9 solution -- the results would've have been virtually identical:  `F4.CODE.SWITCH` would have knocked both `BIRTHSEX` and `HEIGHT2` out of significance in the 'fully loaded' model (where adjustments were made for Aprons, Nurse Cooperation, Training with Sensitivity, Respect/Confidence, Tactile Training, and Training Level).  And if `F4.CODE.SWITCH` is removed from the 'fully-loaded' model, then just as before, `BIRTHSEX` prevails over its highly correlated covariate `HEIGHT2,` *however* it does *not* retain significance even in the presence of all the aforementioned covariates ("interventions"), as its coefficient falls from its original, unadjusted 0.45 to 0.27, p= 0.060 (a "trend").  While not shown, the multilevel variable `HEIGHT2` is *not* significant in this final model, according to the Type 3 Anova table.

<br>
<br>
  
![Regressions:  BIRTHSEX predicting PAIN/INJURY with HEIGHT2, as captured in the survey](images/Promax 9 - Code Switch Regressions 2.png){width=1600px}

<br>
<br>

While maintaining the correlation among our factors by using an orthogonal rotation in our EFA (a Promax rotation in this case) allows us to model the effects that one or more factors might have on our outcome-of-interest factor (F9.PAIN), doing so can sometimes create issues with multicollinearity in our regression models -- and this is something we would certainly want to test for by looking at the Variance Inflation Factors (VIFs).

<br>

```{r VARIANCE-INFLATION-FACTOR-TESTS, figure.align= 'center', fig.dim= c(16,12)}

library(performance)

fullyloaded.lm <- lm( F9.PAIN ~ BIRTHSEX + HEIGHT2 + F4.CODE.SWITCH + F7.APRONS.LW  + F3.NURSE.COOP + F1.TRNG.FEEDBACK.SENSITIVITY + F2.RESPECT.CONFIDENCE + F5.TRNG.TACTILE +                                      TRAINING_LEVEL, data = FACTANAL.RIGOROUS.fa)

check_collinearity(fullyloaded.lm)[, c(1:6)] 

plot(check_collinearity( fullyloaded.lm ))
...
- Our ***fully-loaded model*** has three categorical variables (`BIRTHESEX`, `HEIGHT2`, and `TRAINING_LEVEL`) along with five additional numeric regressors (`Factor1 - Training Feedback & Sensitivity`, `Factor2 - Respect & Confidence`,  `Factor3 - Nurse Cooperation`, `Factor5 - Tactile Training`, `Factor7 - Lightweight Aprons`, and our explanatory factor-of-interest `Factor4 - Code Switching`).

- Based on the above VIF tests, all of our *factors* have VIF values well below 5 -- in fact all of them are below 1.5.  Two of our three categorical regressors, `BIRTHSEX` and `HEIGHT2` have VIFs that are slightly higher (2.3 and 2.6, respectively), which is not surprising given how closely correlated these two variables are (*r= * 0.71).  But on balance, based no the output above, what would likely be our *final* model does not appear to have any issues with multicollinearity.  (Our *final* model has a very respectable R-squared value, and we have ensured that this model controls the explanatory variable `Code Switching` for other variables that could reasonably explain any noted differences in the the `Pain` factor, our outcome-of-interest.)

<br>
<br>

#### Tests to Confirm that we have not "overfitted" our model

<br>

Our ***fully-loaded*** model (with 3 categorical independent variables and an additional 6 independent covariates) has been checked for and passed our multicollinearity tests.  While it has a very respectable R-squared value (32.3/27.6), it might be interesting to load *all* the covariates that we considered into the `glmulti` package to see what model it would recommend based on the AIC value (Akaike Information Criterion), a metric often used to gauge the fit of regression models.  With this package we can highlight, for example, the 100 best fitting models and then see whether the model we've chosen falls within the range of these models -- and if it doesn't, we can determine just how different the suggested models differ from ours.
...
```{r MODEL-FIT-TESTS1, warning=F}


require(glmulti) #alternative to stepwise 
require(pracma)
require(flextable)
...
x <- glmulti( F9.PAIN ~ BIRTHSEX + HEIGHT2 + HTNUM + F4.CODE.SWITCH + F7.APRONS.LW  + F3.NURSE.COOP + F1.TRNG.FEEDBACK.SENSITIVITY + 
                        F2.RESPECT.CONFIDENCE + F5.TRNG.TACTILE + TRAINING_LEVEL,
         data = FACTANAL.RIGOROUS.fa,
         crit = aicc,
         level=1,
         method= 'g',  # use method =d to see how many models level=1 (no interactions) vs. level=2 (interactions) will produce
         fitfunction = 'lm',
         plotty=F,
         report=F,
...
print(x)  #prints best model
plot(x)
weightable(x)[1:5, ] %>% flextable()  #prints five best models
...
- Based strictly on the AIC score, a model that controls `BIRTHSEX` and `Code Switching` for Nurse Cooperation, Training with Sensitivity, Respect & Confidence, and Tactile Training provides the best fit, with an AIC of 521.14.  
- The range of the top 100 models runs from 521.14 to 531.37. Does our fully-loaded model fall within this range?  If not, is that a concern?

<br>
<br>

```{r MODEL-FIT-TESTS2, warning=F}
  


#Compare 3 models  using new package 

suppressMessages(library(AICcmodavg))



lm.BestAIC <- lm( F9.PAIN ~ BIRTHSEX + F4.CODE.SWITCH + F3.NURSE.COOP + F1.TRNG.FEEDBACK.SENSITIVITY +  F2.RESPECT.CONFIDENCE + F5.TRNG.TACTILE,
                    data = FACTANAL.RIGOROUS.fa)


lm.BestAIC.withSexHeight <- lm(F9.PAIN ~ BIRTHSEX + HTNUM + F4.CODE.SWITCH + F3.NURSE.COOP + F1.TRNG.FEEDBACK.SENSITIVITY + F2.RESPECT.CONFIDENCE +                                       F5.TRNG.TACTILE, data = FACTANAL.RIGOROUS.fa)

lm.fully.loaded <- lm( F9.PAIN ~ BIRTHSEX + HEIGHT2 + F4.CODE.SWITCH + F7.APRONS.LW  + F3.NURSE.COOP + F1.TRNG.FEEDBACK.SENSITIVITY + 
                         F2.RESPECT.CONFIDENCE + F5.TRNG.TACTILE + TRAINING_LEVEL, 
                         data = FACTANAL.RIGOROUS.fa)

mod.set <-  list(lm.BestAIC, lm.BestAIC.withSexHeight, lm.fully.loaded)
mod.names <- c('Best AIC','Best AIC with Sex & Height','Fully-Loaded')


aictab( cand.set= mod.set, modnames= mod.names ) %>% flextable()
...
- Based the comparison output from the `AICcmodavg` package, while our fully-loaded model has an AIC that is about 8 points higher than the model that what was deemed as having the best fit, its AIC score is still under the upper limit of the top 100 models (based on fit) from the `glmulti` package.  If the research team decides that the higher AIC is an appropriate trade-off because we *need* to control for sex, height and training level, then it appears as if we do not have to concern ourselves that our model is "too fitted" to our data.  


- However if a more parsimonious model is desired, then the recommended model still allows us to tell our story:  Reduction in reports of Transient Pain can be reduced in "Code Switchers" if we add lightweight aprons, cooperative nurses, train with respect and inspire confidence among trainees, and train with feedback and sensitivity -- and this all holds true even when controlled for sex.  Additionally, one of the nice features of this particular model is that both `BIRTHSEX` and `F4.CODE.SWITCH` variables maintain their significance in the presence of the other covariates.  The `BIRTHSEX` unadjusted coefficient of 0.45 falls to 0.25 (*p= * 0.0122), and the unadjusted `F4.CODE.SWITCH` coefficient falls from 0.32 to 0.13 (*p= * 0.0191).

<br>


```{r MODEL-FIT-TESTS3, warning=F, fig.dim= c(16,12)}

#Model Stats for Model with Best AIC

summary( lm.BestAIC )

check_model( lm.BestAIC )
