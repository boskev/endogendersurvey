...
ggplot(  SUBSET2, aes(x= HTCATSEX)) + facet_grid( SUBSET2$BIRTHSEX ) +  
  ...
  theme_538()  +
  xlab(paste0("Sex-Adjusted Height Bands","\n","(Female AVG = Band 3, 5'4-5'6  * Male AVG = Band 5, 5'8-5'10)"))+ ylab("Counts") 
...
```{r TASKS-CORRELATION-PARITALS2, warning=F }
...
# Pearson Correlations: INJURY with GLOVE, with and wihtout partialling out SEX
...
x <-  tibble::rownames_to_column( rbind(e,p), 'STATISTIC')
x
...
y <-  tibble::rownames_to_column( rbind(e, p), 'STATISTIC')
y
...
# Pearson Correlations: TRANSIENT HAND PAIN with GLOVE, with and wihtout partialling out SEX


# TP_HAND x GLOVE

e <-NEWCORRS %>% 
  select( GLOVE, TP_HAND) %>% 
  summarize_each(funs(cor.test(.,  NEWCORRS$GLOVE)$estimate)) %>%  `rownames<-`("Pearson r")
...
# TP_HAND x GLOVE, controlling for Sex 
...
# Pearson Correlations: TRANSIENT PAINS (alll) with HEIGHT, with and wihtout partialling out SEX
...
# TPAINS x HEIGHT, controlling for Sex 
e <-NEWCORRS %>% 
  select( HEIGHT,  starts_with("TP_")) %>% 
