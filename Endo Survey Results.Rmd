...
CORRMATRIX <-
  ...
  mutate( SEX = case_when(
                              # is.na(BIRTHSEX) ~ NA_real_,
                              BIRTHSEX == "M" ~ 0,
                                 TRUE ~ 1 ) ,
          
          RACE = case_when( RACE2 == "WHITE" ~ 0,
                            TRUE  ~ 1),
          
          AGECAT = case_when( AGE2 == "< 30" ~ 1,
                              AGE2 == "30-34" ~ 2,
                              AGE2 == "35-40" ~3,
                              TRUE ~ 4),
          
          TRNG_LEVEL = case_when( TRAINING_LEVEL == 'First Year' ~ 1,
                                  TRAINING_LEVEL == 'Second Year' ~ 2,
                                  TRAINING_LEVEL == 'Third Year' ~ 3,
                                  TRUE  ~ 4) ,
          
          HEIGHT = case_when( HEIGHT2 == "< 5" ~ 1,
                              HEIGHT2 == "5'1-5'3" ~ 2,
                              HEIGHT2 == "5'4-5'6" ~ 3,
                              HEIGHT2 == "5'7-5'9" ~ 4,
                              HEIGHT2 == "5'10-6'" ~ 5,
          ...
          INJURY = case_when(  EVER_INJURED== "Y" ~ 1,
                                    TRUE ~ 0)  ,
          
          TPAIN =  case_when(     EXPERIENCED_TRANSIENT_PAIN_HAND == "Y" |
          ...
          ERGO_OPTIM =  case_when( ERGO_OPTIMIZATION == "Y" ~ 1,
                                    TRUE ~ 0),
                                   
          SENSITIVE_TEACHER = case_when (TEACHER_SENSITIVITY_STATURE_HANDSIZE == "Y" ~ 1,
                                          TRUE ~ 0),
          
          FNAME_NP =  case_when (FIRST_NAME_NO_PERMISSION == "Y" ~ 1,
                                 TRUE ~ 0)) %>% 
...
#### Task 2: How are the associations/correlations for certain variables affected if we statify them by Training Level?
...
CORRMATRIX %>%
  ...
  grouped_ggcorrmat(
    ...
    annotation.args = list(
      tag_levels = "1",
      title = "Relationship between Variables across Training Level"
    ),
    p.adjust.method = "none"
  )
...
##### There apprears to be a few slightly significant correlations that might be worth exploring with simple regression:

- First Year: Transient Pain with both Ergo Optimization and Sensitive Teacher
- Second Year: First Name & Ergo Optimization (???)
- Advanced:  Transient Pain & Ergo Optimization
...
CORRMATRIX %>%
  ...
  summary(lm( TPAIN ~ ERGO_OPTIM, data= TLEVEL))
  summary(lm( TPAIN ~ ERGO_OPTIM + relevel(TRNG_LEVEL, ref="Advanced"), data= TLEVEL ))
  car::Anova(lm( TPAIN ~ ERGO_OPTIM + TRNG_LEVEL, data= TLEVEL ), type="III")
  
```

<br>

- **Conclusion:**  Ergonomic Optimization is a significant effect before and after adjust for Training Level, which itself is *not* a significant effect. (Beta for EO decreases from -0.125 to -0.114, meaning that as Ergo Optimization increases, it causes Transient Pain reports to decline.) However, when compared to the reference category ("Advanced"), the impact on Second Year residents is a trend.
...
#2. Does having a Sensitive Teacher with concern for your size/stature influence reports of Transient Pain, with and without adjusting for Training Level?
  ...
  summary(lm( TPAIN ~ SENSITIVE_TEACHER, data= TLEVEL))
  summary(lm( TPAIN ~ SENSITIVE_TEACHER +  relevel(TRNG_LEVEL, ref="Advanced"), data= TLEVEL ))
  car::Anova(lm( TPAIN ~ SENSITIVE_TEACHER + TRNG_LEVEL, data= TLEVEL ), type="III")
  
```
  
<br>
  
- **Conclusion:**  Sensitive Teacher is a slightly significant effect before and after adjust for Training Level, which itself is *not* a significant effect. (Beta for ST increases slightly from -0.0925 to -0.0978, meaning that as Teacher Sensitivity increases, it causes Transient Pain reports to slightly decline.) However, when compared to the reference category ("Advanced"), the impact on Second Year residents is a very significant (p= 0.0248)
...
```{r TASKS-CORRELATIONS-TRAINING-LEVEL-EXPLORATORY-REGRESSION3 } 
  ...
  #3. Does having a Sensitive Teacher with concern for your size/stature influence the use/encouragment of Ergonomic Optimizaiton, with and without adjusting for Training Level?
  
  summary(lm( ERGO_OPTIM ~ SENSITIVE_TEACHER, data= TLEVEL))
  summary(lm( ERGO_OPTIM ~ SENSITIVE_TEACHER +  relevel(TRNG_LEVEL, ref="Advanced"), data= TLEVEL ))
  car::Anova(lm( ERGO_OPTIM ~ SENSITIVE_TEACHER +  TRNG_LEVEL, data= TLEVEL ), type="III")
  
```
  
<br>

- **Conclusion:**  Sensitive Teacher is a highly significant effect before, and an extremely highly significant effect after adjusting for Training Level, which itself is a trend (almost significant). (Beta for ST increases slightly from +0.186 to +0.204, meaning that as Teacher Sensitivity increases, reports of Ergonomic Optimization also rise.)  When compared to the reference category ("Advanced"), *ALL* other training levels are significantly different (i.e., lower).  Does this suggest that teachers are more likely to be sensitive to ergonomics in the Endoscopy Suite when dealing with older trainees, or does it suggest that younger trainees don't really know what proper ergonomics should entail and how to rate their instructors on this metric?
