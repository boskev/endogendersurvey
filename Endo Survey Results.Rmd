...
#### <span style="color:red">[Click here to see the latest Oblique Factor Analysis Solutions](#Oblique)</span>
...
- The factor `F4.CODE.SWITCH` is highly correlated with SEX (females), and HEIGHT (shorter trainees), and less correlated with TRAINING_LEVEL (where, curiously, it seems to be more positively associated with Advanced trainees).  It is *not* correlated at all with RACE, however (where RACE is a binary variable, Whites vs. Non-Whites -- it is higher among Non-Whites, but not statistically).
