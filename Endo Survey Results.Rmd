...
Since what constitutes a "moderate effect size" can change depending on the type of statistical test being performed, below are the results of a power calculation for a two-tailed T-test, ANOVA (four-level categorical variable),  Pearson correlations, and the F-test from multivariate regression analyses:
...
- **Interpretation:**  `TRAINING_LEVEL` is *not* a significant effect in predicting the likelihood of Injury `EVER_INJURED`.  Despite the facts that (1) the prevalence of self-reported injuries *increases* with each step of `Training Level` (First Years are the least likely to be injured (OR=1.09), compared to Second Years (OR=1.23) and Advanced respondents (OR=1.22); and (2) Second Years & Advanced respondents have a higher likelihood of injury when compared against all other groups, none of these results is statistically significant.
