...
- For the sake of brevity, the results of only two nine-factor `Promax` solutions are displayed below.  Each has passed certain key model fit and consistency/reliability tests:
    ...
    - Each solution explains at least 42% of the overall variance
    - Each solution has a Tucker-Lewis Index (TMI) greater than 0.92 and a Root Mean Standard Error Approximation (RMSEA) less than 0.3
...
#### Promax Solution 1:  9-Factors (41 of 54 variables included in final factor load)

<br>
<br>


![MLE Factor Analysis: Promax Rotation with 9 factors produced from 41 questions](images/Promax9-41Q.png)
<br>
<br>

![Correlation Matrix for Promax-9-DialExt](images/PA Factor Analysis - 9-Factor PROMAX Corrmatrix (Median Substitution DIALEXT).png)


<br>

- What makes this solution interesting is that it highlights just how much of what transpires in the Endoscopy Suite hinges on the factor tentatively named **Respect-EStaff** (Respect Endoscopy Suite Staff).
...
- By expanding to nine factors, not only did we achieve optimal TMI and RMSEA values (i.e., better model fit), we saw a schism in two factors from one of our Varimax-rotated solutions:  "Apron Fit & Availability" produced two distinct, reliable factors related to "Standard Aprons" and "Lightweight Aprons"; and "Respect Senior Staff" spun off a second factor more aptly described as "Trainee Confidence."
...
![Regressions:  BIRTHSEX predicting PAIN/INJURY](images/Promax 9 - Dial Ext Regressions.png){width=1600px}

<br>

- The regression summary above charts what happens to the coefficient of the variable BIRTHSEX when it is used to predict the factor PAIN.  Unadjusted, the coefficient starts at 0.41. The next six models show what happens if only *one* intervention is made:  For example, adding a *standard* apron increases the coefficient to 0.43 (i.e., increases the likelihood of PAIN). But the next single-step interventions (red underscores) - using a lightweight apron, improving trainee confidence, treating trainees with respect, having nurses be more cooperative, and training with sensitivity and appropriate feedback - each reduces the coefficient to as low as 0.34.   In the next models (blue underscores), we see what happens when we (1) include *all* the aforementioned interventions (coefficient drops to 0.30); (2) include the Tactile Training factor, which is positively correlated with the Pain factor (coefficient drops further to 0.27); and (3) adjust further for HEIGHT (which is so correlated with BIRTHSEX that it must be included to confirm a true sex effect), and Training Level.  The final model, which includes all these covariates, has a coefficient of 0.31 and our variable of interest, BIRTHSEX, has maintained its significance throughout (whereas HEIGHT and TRAINING_LEVEL did not, according to the Type 3 Anova table, not shown).
- <b>Final Takeaway:</b>  Training with Sensitivity + Respect from Sr. ES Staff + Cooperative Nurses + Lightweight Aprons signficiantly
...
- This solution is very similar to the one above, except that it includes two questions that were originally ignored due to their perceived limited scope (i.e., questions regarding whether trainees were more or less likely to use Dial Extenders in front of a male or female instructor, as 70% of trainees reported not even using Dial Extenders).  With the addition of these two new questions, and by holiding the number of factors requested constant at nine factors, we noticed that the "Respect" and "Trainee Confidence" factors were "reunited" to form one solid, reliable factor (identical to what was observed in the previously discussed Varimax-rotated solutions).  It should be noted that the scoring of these questions included *all* subjects, not just those reporting having used Dial Extenders -- so *every* respondent contributed in some way to the factor loadings and scores involving these new two questions.
- By including these two questions, we may have stumbled upon an important factor that can be best described as `"Code Switching."`  While the term is typically used in the context of cultural linguistics, "Code Switching" here is meant to imply that some trainees may act one way in front of one audience (i.e., "their people") and another way in front of perceived figures of authority (or simply in front of instructors who are different from them).
...
- The factor `F4.CODE.SWITCH` is highly correlated with SEX (females), and HEIGHT (shorter trainees), and less correlated with TRAINING_LEVEL (where, it seems to be more positively associated with First-Year & Second-Year trainees -- "Advanced" is the reference category, and as such, its bar appears first in the histogram).  It is *not* correlated at all with RACE, however (where RACE is a binary variable, Whites vs. Non-Whites -- it is higher among Non-Whites, but not statistically).
...
- If we were to use the resulting odds ratios to inform the profile of these so-called "Code Switchers," we might find that they are . . . .
    ...
    - 3.16 times more likely to feel that they were ***not*** **trained with sensitivity or with adequate feedback**
...
- The first three models establish a baseline coefficient for each of these three variables: BIRTHEX = 0.43, HEIGHTBAND = 0.40 and F4.CODE.SWITCH = 0.30.
- The next two models adjust BIRTHSEX and HEIGHTBAND for `F4.CODE.SWITCH,` and in each case, while all variables remain statistically significant, the p-values for BIRTHSEX and HEIGHTBAND rose, while that of the Code Swich factor remained very low (p < 0.001).  In the next model, where all three variables have PAIN regressed on to them, *only* `F4.CODE.SWITCH` remains statistically significant (p < 0.001), suggesting that in this three-way battle, it is the Code Switch factor that is strongest predictor of transient pain/injury.
- Similar to what was done previously, the next models (blue underscore) add single interventions (lighweight Aprons, Respect, Cooperative Nurses, etc.), each of which drops the coefficient for `F4.CODE.SWITCH` from its 0.30 high to as low as 0.20 (and this is with BIRTHSEX and HEIGHTBAND also included in the model).
- In the two "fully loaded" models -- one with Tactile Training and one without -- the effect of `F4.CODESWITCH` remains statistically significant, and its coefficient drops as low as 0.13

<br>

- <b>NOTE:  </b><br>The variable `HEIGHTBAND` (a binvary variable, where 0=Short and 1=Tall) was used in the above regressions for the sake of simplicity.  (Having only two levels, `HEIGHTBAND` makes tracking the effects of height easier.)  But if the *original* multilevel categorical variable `HEIGHT2` was used in these models instead -- similar to what was done with the previous Promax-9 solution -- the results would've have been virtually identical:  `F4.CODE.SWITCH` would have knocked both `BIRTHSEX` and `HEIGHT2` out of significance in the 'fully loaded' model (where adjustments were made for Aprons, Nurse Cooperation, Training with Sensitivity, Respect/Confidence, Tactile Training, and Training Level).  But if `F4.CODE.SWITCH` is removed from the 'fully-loaded' model, then just as before, `BIRTHSEX` prevails over its highly correlated covariate `HEIGHT2,` retaining its significance even in the presence of all the aforementioned covariates ("interventions"), with its coefficient falling from 0.43 to 0.28, p= 0.048.  While not shown, the multilevel variable `HEIGHT2` is *not* significant in the final model, according to the Type 3 Anova table.
