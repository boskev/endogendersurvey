...
plot_xtab(SURVEY$TRAINING_LEVEL,SURVEY$EVER_INJURED,  margin = "row", 
  ...
  #Test to see whether Training Level is a significnat effect overall:
...
- The following factor solutions are based on *oblique* rotations, ones that allows the factors to retain any correlation they may exhibit among each other
...
- For the sake of brevity, the results of only two nine-factor `Promax` solutions are displayed below.  Each has passed certain key model fit and consistency/reliability tests:
    ...
    - Each solution has an overall KMO greater than 0.65
    ...
    - The individual factors themselves for each solution were evaluated for consistency/reliability, and none scored lower than 0.60 on any of the three aforementioned tests
...
```{r PLAYGROUND-CODESWITCH,   fig.align='center', fig.dim= c(12,12)}
...
- Since all factors have a mean of zero, it is simple to convert them to binary variables, where 0=No and 1=Yes, relative to each factor, making running logistic regessions on these factors quite simple
- If we were to try to use odds ratios to predict the profile of these so-called "Code Switchers," we might find that the . . ..
    ...
    - The odds of them trending Non-White is 1.28 (although this result is not statistically significant)
...
#### Question 6:  What happens if we use <span style="color:red">Principal Component Analysis</span> instead as a means of data reduction?
...
```{r PLAYGROUND-PRINCOMP1, warning=F }
  ...
  print(PRINCOMP_FINAL$loadings, digits=2, cutoff=.4, sort=T)
  print(PRINCIPAL_FINAL$PVAL)

  
```
```{r PLAYGROUND-PRINCOMP2, warning=F, fig.align='center', fig.dim= c(16,12)}
...
```{r PLAYGROUND-PRINCOMP3, warning=F }
