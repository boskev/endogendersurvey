...
- Teachers train with sensitivity to size/stature (Yes=1, No=0)
...
RESPECT <-
  ...
  mutate( BIRTHSEX2 = case_when( BIRTHSEX == "M" ~ 0,
          ...
          RESPECT_GPAINS = case_when (GROWING_PAINS  == "Y" ~ 1,
                                 TRUE ~ 0),
          
 
          RESPECT_SENSITIVITY = case_when (TEACHER_SENSITIVITY_STATURE_HANDSIZE  == "Y" ~ 1,
                                 TRUE ~ 0),
  ...
  mutate(RESPECT_SCORE = sum( RESPECT_NURSES + RESPECT_TECHS + RESPECT_NFREQ + RESPECT_ESTAFF + RESPECT_ANES + 
                           RESPECT_GIATT + RESPECT_SENSITIVITY + RESPECT_FNAME)) %>% 
  
  select( BIRTHSEX, BIRTHSEX2, AGE2, RACE, RACE2, HEIGHT, HTCAT, TRAINING_LEVEL, EVER_INJURED, starts_with("RESPECT_") )
...
#If Respect Score includes "Teacher Sensitity to Stature/Hand Size"
...
respect.birthsex.lm.adj   <- lm( RESPECT_SCORE ~ BIRTHSEX + EVER_INJURED , data= RESPECT)
  ...
  tab_model( respect.birthsex.lm.unadj,
             ...
             dv.labels= c('Birth Sex Unadj','Height Cat UnAdj Training Level', 'Birth Sex Adjusted Ever Injured'))
